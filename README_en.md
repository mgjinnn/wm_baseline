...
2) Do not directly return the watermark in wms.npy in the watermark extraction stage, and extract the embedded watermark by developing a watermark extraction algorithm. If the watermark in wms.npy is returned directly in the extraction stage, the result will be invalid and the qualification will be disqualified.<br/>
...
5) When running the image, any network in the container is unavailable, please install the dependent software and packages in the image.<br/>
6) In order to allocate resources reasonably, the running time of a single submission cannot exceed 1 hour, and the program will automatically stop after exceeding it, and the running results will be failed.<br/>
7) Make sure that the cp command is available in the image.<br/>
8) The Docker image should not be larger than 8 GB, and it should not contain any datasets or video data generated by the code.<br/>
9) PSNR is an objective metric that is used to filter out bad video quality. Subjective assessment will be used in the final ranking. The ranking will be canceled or adjusted backward if the quality of the watermarked video is obviously low.<br/>
<br/>
